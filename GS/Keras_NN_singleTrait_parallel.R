# Build NN regressor, parallel. 
# activations : relu | elu | selu | hard_sigmoid | sigmoid | linear | softmax | softplus | softsign | tanh | exponential
# optimizer: adam | adamax | adadelta | adagrad | nadam | rmsprop 
Keras_NN_singleTrait <- function(feed_set,
                                 batch_size=256, epochs=200,
                                 patience=NULL,
                                 modelIn=NULL,
                                 activation='sigmoid',
                                 optimizer='rmsprop', #optimizer_rmsprop(),
                                 include_env=TRUE,
                                 isScale=TRUE,
                                 isScaleSNP=TRUE,
                                 pn=c(1,4), # manual
                                 dropout=.2,
                                 nlayer=5,
                                 weight_snp=NA, Del0SNP=F, Ntt=seq(1,10)) {
  library(dplyr)
  library(parallel)
  library(foreach)
  library(doParallel)
  nam_ph <- colnames(feed_set[[1]][[1]][[1]][[1]][[2]])
  num_ph <- length(nam_ph)
  num_tt <- length(Ntt)
  if (is.na(pn)) { pn <- 1:num_ph }
  
  result <- list()
  pp <- 1
  while (pp <= length(pn)) {
    p <- pn[pp]
    cores <- detectCores() - 2
    if (num_tt < cores) { cores = num_tt }
    cl <- makeCluster(cores)
    registerDoParallel(cl)
    tmp_tt <- foreach(tt = 1:num_tt) %dopar% {
      tx <- Ntt[tt]
      source('~/gs/rice/Keras_NN_singleTrait_faster.R')
      NN_ttX(tx, p, feed_set, batch_size, epochs,
             patience, modelIn,
             activation, optimizer,
             include_env,
             isScale, isScaleSNP,
             dropout, nlayer,
             weight_snp, Del0SNP)
    }
    stopCluster(cl)
    result_t_mx <- matrix(NA, num_tt, 5,
                          dimnames = list(Ntt, c("MAE_min","MAE_mean","loss_min","loss_mean","Cor_max")))
    nam_tt <- c()
    for (rtt in 1:num_tt) {
      result_t_mx[rtt, ] <- c(min(tmp_tt[[rtt]][[3]]), mean(tmp_tt[[rtt]][[3]]),
                              min(tmp_tt[[rtt]][[2]]), mean(tmp_tt[[rtt]][[2]]),
                              max(tmp_tt[[rtt]][[4]]))
      nam_tt <- c(nam_tt, paste("random_tt_", Ntt[rtt], sep = ""))
    }
    tmp_tt[[length(tmp_tt)+1]] <- result_t_mx
    tmp_tt[[length(tmp_tt)+1]] <- list(MAE_min=(t.test(result_t_mx[, 1]))$p.value,
                                       MAE_mean=(t.test(result_t_mx[, 2]))$p.value,
                                       loss_min=(t.test(result_t_mx[, 3]))$p.value,
                                       loss_mean=(t.test(result_t_mx[, 4]))$p.value,
                                       Cor_max=(t.test(result_t_mx[, 5]))$p.value)
    names(tmp_tt) <- c(nam_tt, "resultMLC", "p_value")
    result[[pp]] <- tmp_tt
    pp <- pp + 1
  }
  names(result) = nam_ph[pn]
  return(result)
}

weighted <- function(dataMx, weight_snp, Del0SNP=F) {
  if (Del0SNP) {
    dataMx <- dataMx[,-which(weight_snp==0)] # Delete SNP with zero effect
    weight_snp <- weight_snp[-which(weight_snp==0)]
  }
  for (colw in 1:ncol(dataMx)) {
    dataMx[,colw] <- dataMx[,colw] * weight_snp[colw]
  }
  return(dataMx)
}

NN_ttX <- function(tx, p, feed_set, batch_size=256, epochs=300,
                   patience=NULL, modelIn=NULL,
                   activation='sigmoid', optimizer='rmsprop', #optimizer_rmsprop(),
                   include_env=TRUE,
                   isScale=TRUE, isScaleSNP=TRUE,
                   dropout=.2, nlayer=5,
                   weight_snp=NA, Del0SNP=F) {
  library(keras)
  library(dplyr)
  if (!is.na(weight_snp)) { wt_snp <- weight_snp[[tx]][p,] }
  num_cv <- length(feed_set[[tx]][[1]])
  # Test set
  if(isScale) {
    test__envi = scale(feed_set[[tx]][[2]][[4]])
    test__phen = scale(as.matrix(feed_set[[tx]][[2]][[2]][, p]))
  } else {
    test__envi = feed_set[[tx]][[2]][[4]]
    test__phen = as.matrix(feed_set[[tx]][[2]][[2]][, p])
  }
  if (isScaleSNP) {
    test__geno = scale(feed_set[[tx]][[2]][[1]], center = FALSE)
  } else {
    test__geno = feed_set[[tx]][[2]][[1]]
  }
  ##############
  if (!is.na(weight_snp)) {
    test__geno <- weighted(test__geno, wt_snp, Del0SNP)
  }
  ##############
  if (include_env) {
    x_test  = as.matrix(cbind(test__envi, test__geno))
  } else {
    x_test  = as.matrix(test__geno)
  }
  y_test  = as.matrix(test__phen)
  input_shape = ncol(x_test)
  
  # CV
  history_list <- list()
  test_loss <- c()
  test_mae <- c()
  test_cor <- c()
  test_prediction <- list()
  cc <- 1
  while (cc <= num_cv) {
    if(isScale) {
      train_envi = scale(feed_set[[tx]][[1]][[cc]][[1]][[4]])
      train_phen = scale(as.matrix(feed_set[[tx]][[1]][[cc]][[1]][[2]][, p]))
      valid_envi = scale(feed_set[[tx]][[1]][[cc]][[2]][[4]])
      valid_phen = scale(as.matrix(feed_set[[tx]][[1]][[cc]][[2]][[2]][, p]))
    } else {
      train_envi = feed_set[[tx]][[1]][[cc]][[1]][[4]]
      train_phen = as.matrix(feed_set[[tx]][[1]][[cc]][[1]][[2]][, p])
      valid_envi = feed_set[[tx]][[1]][[cc]][[2]][[4]]
      valid_phen = as.matrix(feed_set[[tx]][[1]][[cc]][[2]][[2]][, p])
    }
    
    if (isScaleSNP) {
      train_geno = scale(feed_set[[tx]][[1]][[cc]][[1]][[1]], center = FALSE)
      valid_geno = scale(feed_set[[tx]][[1]][[cc]][[2]][[1]], center = FALSE)
    } else {
      train_geno = feed_set[[tx]][[1]][[cc]][[1]][[1]]
      valid_geno = feed_set[[tx]][[1]][[cc]][[2]][[1]]
    }
    ##############
    if (!is.na(weight_snp)) {
      train_geno <- weighted(train_geno, wt_snp, Del0SNP)
      valid_geno <- weighted(valid_geno, wt_snp, Del0SNP)
    }
    ##############
    
    if (include_env) {
      x_train = as.matrix(cbind(train_envi, train_geno))
      x_valid = as.matrix(cbind(valid_envi, valid_geno))
    } else {
      x_train = as.matrix(train_geno)
      x_valid = as.matrix(valid_geno)
    }
    y_train = as.matrix(train_phen)
    y_valid = as.matrix(valid_phen)
    
    # Define and Refresh NN for CV
    if (is.null(modelIn)) {
      model = keras_model_sequential() %>%
        layer_dense(units = 256, activation = activation, input_shape = input_shape) %>% 
        #layer_dropout(rate = dropout) %>% 
        layer_dense(units = 128, activation = activation) %>%
        layer_dense(units = 32, activation = activation) %>%
        layer_dense(units = 1)
    }
    if (!is.null(modelIn)) {
      layer1_units=512
      layer2_units=256
      layer3_units=128
      layer4_units=64
      if (nlayer == 4) {
        modelIn = keras_model_sequential() %>%
          layer_dense(units = layer1_units, activation = activation, input_shape = input_shape) %>% 
          layer_dense(units = layer2_units, activation = activation) %>%
          layer_dropout(rate = dropout) %>%
          layer_dense(units = layer3_units, activation = activation) %>%
          layer_dense(units = 1)
      }
      if (nlayer == 5) {
        modelIn = keras_model_sequential() %>%
          layer_dense(units = layer1_units, activation = activation, input_shape = input_shape) %>% 
          layer_dense(units = layer2_units, activation = activation) %>%
          layer_dropout(rate = dropout) %>%
          layer_dense(units = layer3_units, activation = activation) %>%
          layer_dense(units = layer4_units, activation = activation) %>%
          layer_dense(units = 1)
      }
      model = modelIn
    }
    
    model %>% compile(
      loss = "mse", #loss = "categorical_crossentropy",
      optimizer = optimizer,
      metrics = list("mean_absolute_error")
    )
    model %>% summary()
    
    if (!is.null(patience)) {
      early_stop <- callback_early_stopping(monitor = "val_loss", patience = patience)
      history_list[[cc]] <- model %>% fit(
        x_train, y_train,
        batch_size = batch_size,
        epochs = epochs,
        verbose = 0,
        validation_data = list(x_valid, y_valid),
        callbacks = list(early_stop)
      )
    } else {
      history_list[[cc]] <- model %>% fit(
        x_train, y_train,
        batch_size = batch_size,
        epochs = epochs,
        verbose = 0,
        validation_data = list(x_valid, y_valid),
      )
    }
    c(loss, mae) %<-% (model %>% evaluate(x_test, y_test))
    test_loss <- c(test_loss, loss)
    test_mae <- c(test_mae, mae)
    test_prediction[[cc]] <- model %>% predict(x_test)
    test_cor <- c(test_cor, cor(test_prediction[[cc]], y_test))
    print(paste(c("loss: ", loss, ";  ", "MAE: ", mae, ";  Cor: ", test_cor[cc]), collapse = ""))
    cc <- cc + 1
  }
  return(list(history=history_list,
              loss=test_loss,
              MAE=test_mae,
              Cor=test_cor,
              test_prediction=list(y_pred=test_prediction, y_test=y_test)))
}
