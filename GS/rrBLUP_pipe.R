...
rrBLUP_for_ph_para <- function(dataSet, isCluster=TRUE, cls=NA) {
  ...
  result_ph <- foreach(ph = 1:num_ph) %dopar% {
    ...
    for (t in 1:num_tt) {
      ...
      result_t_mx[t, 2] <- result_t[[t]][[3]]
      result_t_mx[t, 3] <- result_t[[t]][[2]]
    }
    ...
    result_ph[[ph]] <- list(result=result_t, resultMMC=result_t_mx,
  ...
  for(ph in 1:num_ph) {
    ...
    for (t in 1:num_tt) {
      ...
      test <- list(trainValidTest_pre[[t]][[2]][[1]],
                   as.matrix(trainValidTest_pre[[t]][[2]][[2]][, ph]))
      #tmp
      #save(list = c("geno","phen","test"), file = paste(c("tmp", ph, t, ".RData"), collapse = "_"), compression_level = 9)
      source('~/gs/rice/rrBLUP_train2test.R')
      result_t[[t]] <- rrBLUP_train2test(train, test)
      result_t_mx[t, 1] <- result_t[[t]][[1]]
      result_t_mx[t, 2] <- result_t[[t]][[3]]
